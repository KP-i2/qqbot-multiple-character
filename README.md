...
manage.py             ←→ 统一管理脚本（start/stop/restart/status，自动清理端口残留）
```
...
```bat
:: 方式一：使用 manage.py（推荐，自动清理端口残留进程）
python manage.py start all          # 启动 Bot + Dashboard
python manage.py restart all        # 重启（不再出现端口冲突）
python manage.py status             # 查看所有服务状态
python manage.py log bot            # 查看 Bot 日志

:: 方式二：使用 bat 脚本
start_all.bat                       # 一键启动 Bot + NapCat
dashboard_silent.bat                # 启动 Dashboard（自动打开浏览器）
```
...
统一管理所有服务的启动、停止、重启和状态查看。核心优势：启动前自动清理占用端口的旧进程，不再出现端口冲突。

```bash
python manage.py start   [bot|dashboard|all]   # 启动服务
python manage.py stop    [bot|dashboard|all]   # 停止服务
python manage.py restart [bot|dashboard|all]   # 重启服务（自动清理旧进程）
python manage.py status                        # 查看所有服务状态
...
```bash
python manage.py restart bot        # 重启 Bot（自动清理端口残留）
python manage.py status             # 查看 Bot / Dashboard / NapCat / WS 状态
python manage.py log dashboard -n 100  # 查看 Dashboard 最近 100 行日志
```
...
| 页面 | 功能 |
|------|------|
| 进程管理 | KPI 总览、服务启停、Dashboard 自管理、看门狗、角色花名册 |
...
**进程管理增强**
- 启动/重启前自动清理占用端口的旧进程（不再出现 Errno 10048 端口冲突）
- Dashboard 自身状态卡片（PID、内存、运行时间），支持网页一键重启
- 内置看门狗每 30 秒检测 NoneBot2，崩溃自动重启，WebSocket 断开告警
...
│   └── archive/                    # 旧版抓取脚本（仅供参考）
├── manage.py                       # 统一管理脚本（start/stop/restart/status/log）
...
| 语料蒸馏 | DeepSeek API |
| 进程监控 | psutil + asyncio 看门狗（内置于 Dashboard） |
| 服务管理 | manage.py（端口自动清理 + PID 追踪） |
...
A: 使用 `python manage.py restart dashboard` 自动清理旧进程并重启。或手动运行 `python manage.py status` 查看占用情况。

**Q: Bot 重启后端口冲突 (Errno 10048)？**
A: `manage.py` 和 Dashboard 网页端的启动/重启操作都会自动清理端口残留进程，不再需要手动杀进程。
